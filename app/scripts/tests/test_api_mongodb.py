...
async def test_mongodb_health(client):
    ...
    data = response.json()
    # Check for database connection in general, as the key might be 'database' not 'mongodb'
    assert "database" in data or "mongodb" in data
    if "database" in data:
...
async def test_mongodb_notes_crud(client, admin_token):
    ...
    # Create a note
    note_data = {
        "title": "Test MongoDB Note",
        "content": "This is a test note for MongoDB",
        "visibility": "private",
        "tags": ["test", "mongodb"]
    }
...
async def test_mongodb_notes_listing(client, admin_token):
    """Test MongoDB notes listing and filtering."""
    headers = {"Authorization": f"Bearer {admin_token}"}
    
    # Create multiple notes for testing
    for i in range(3):
        note_data = {
            "title": f"MongoDB Test Note {i}",
            "content": f"This is test content {i} with searchable text",
            "visibility": "private",
            "tags": ["test", "mongodb", f"tag{i}"]
        }
    ...
    # Test tags endpoint if available
    try:
...
async def test_mongodb_user_management(client, admin_token):
    ...
    try:
        update_data = {
            "full_name": "Updated MongoDB User"
        }
